...
contract BUNDAU is ERC20Detailed {
  ...
  string constant tokenName = "BUNDAU";
  string constant tokenSymbol = "BDA";
  ...
  //1 percent is the maximum value to burn
  ...
  function lock_erc(uint256 value_, uint256 releaseTime) onlyOwner external returns (uint256) {
    ...
    balances[msg.sender] =_balances[msg.sender].sub(value);
    lockbox_arr.length++;
  ...
  function release_erc(uint256 lockbox_no) onlyOwner public returns(bool){
    ...
    if(releaseTime < now){
      balances[msg.sender] = _balances[msg.sender].add(value);
      status = true;
      lb.value_ = 0;
    }
    return status;
